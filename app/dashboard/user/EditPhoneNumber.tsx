...
function EditPhone() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const res: any = await apiClient.put("user", {  phone: input });
    if (res.ok) {
      setLoading(false);
      toast.success("Phone Number updated successfully");
      // window.location.reload();
    } else {
      console.error(res.data.error || "Failed to update phone number");
      setLoading(false);
      toast.error(res.data.error || "Failed to update phone number");
    }
  };
