...
function EditAccountName() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const res: any = await apiClient.put("user", { accountName: input });
    if (res.ok) {
      setLoading(false);
      toast.success("Account name updated successfully");
      // window.location.reload();
    } else {
      console.error(res.data.error || "Failed to update account name");
      setLoading(false);
      toast.error(res.data.error || "Failed to update account name");
    }
  };
